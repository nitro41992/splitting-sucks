  ...
  Future<String> saveReceipt({
    ...
    try {
      ...
      if (receiptId == null) {
        ...
        docRef = await _receiptsCollection.add(data);
        ...
        if (!snapshot.exists) {
          ...
          data['metadata']['created_at'] = FieldValue.serverTimestamp(); // Add created_at for new doc
          await docRef.set(data); // Create
          ...
          data['metadata'].remove('created_at'); // Prefer server-set created_at, don't overwrite
          await docRef.set(data, SetOptions(merge: true)); // Update/Merge
