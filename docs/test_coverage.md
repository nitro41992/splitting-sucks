...
*   **`WorkflowStepIndicator` (`lib/widgets/workflow_steps/workflow_step_indicator.dart`)**
    ...
    *   **Test Cases:**
        *   Renders the correct number of step indicators based on `stepTitles`.
        *   Highlights the `currentStep` correctly.
        *   Tapping a step indicator (will need to simulate `onTapUp` in `GestureDetector` within `_WorkflowModalBodyState` or test the indicator's direct tap callback if it had one).
...
*   **`ReceiptsScreen` (`lib/screens/receipts_screen.dart`)**
    ...
    *   **Mocks:** `FirestoreService`, `WorkflowModal.show` (potentially using `TestWidgetsFlutterBinding.instance.registerサービス` for static method mocking if complex, or refactoring to make it instance-based for easier mocking).
