...
- **UI/Functionality Bugs:**
    ...
    - ✅ **Investigate Root Cause of Calling Context Unmount in `WorkflowModal.show`:** Resolved by refactoring `main.dart` to ensure a single root `MaterialApp` and using `IndexedStack` for `MainNavigation`, stabilizing the context. If the 'Context unmounted' log (now clarified) for `WorkflowModal.show` appears frequently, investigate the calling screen's lifecycle to prevent its premature disposal during modal pre-flight async operations.
...
- **Workflow Steps (Modal):**
    ...
    - Summary: ✅ (Subtotal calculation, tip/tax persistence fixed). ✅ Fixed "sum of parts" calculation discrepancy and incorrect warning banner by ensuring `Person.sharedItems` are correctly populated for `SplitManager` during Summary step initialization.
